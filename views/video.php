...
if (isset($_SESSION['login']) && $_SESSION['login'] === true) {
    ...
    $conn->insertQuery("INSERT INTO historique (hist_vd_id, hist_mbr_id) 
                                    VALUES (?, ?)", $histoArray);
    $favInfo = [];
    array_push($favInfo, htmlspecialchars($_GET['lessonid']), htmlspecialchars($_SESSION['usr_id']));
    $query = "SELECT IF( EXISTS( SELECT fav_vd_id FROM favorite WHERE favorite.fav_vd_id = ? AND favorite.fav_mbr_id = ?), 1,0) AS favResult";
